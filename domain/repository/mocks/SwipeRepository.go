...
// AddSwipeEntry provides a mock function with given fields: ctx, swipe
func (_m *SwipeRepository) AddSwipeEntry(ctx context.Context, swipe entity.Swipe) error {
	ret := _m.Called(ctx, swipe)

	if len(ret) == 0 {
		panic("no return value specified for AddSwipeEntry")
	}

	var r0 error
	if rf, ok := ret.Get(0).(func(context.Context, entity.Swipe) error); ok {
		r0 = rf(ctx, swipe)
	} else {
		r0 = ret.Error(0)
	}
